...
void LocalMapping::CreateNewMapPoints()
{
    ...
    for(size_t i=0; i<vpNeighKFs.size(); i++)
    {
        ...
        // 步骤5: 通过 BoW 减少极线约束限制匹配时的搜索范围, 加速进行特征匹配
        ...
        // 步骤6: 对每对匹配通过三角化生成 3D 点, 和 Triangulate 函数差不多
        ...
        for(int ikp=0; ikp<nmatches; ikp++)
        {
            ...
            // 步骤6.3: 对于双目, 利用双目得到视差角 (这里只是求解一个近似值 ???)
            if(bStereo1)      //双目，且有深度
            ...
            // 步骤6.4: 三角化恢复 3D 点
            cv::Mat x3D;
            // cosParallaxRays > 0 && (bStereo1 || bStereo2 || cosParallaxRays < 0.9998) 表示视差角正常
            // cosParallaxRays < cosParallaxStereo 表明视差角很小
            // 视差角很小时用三角法恢复 3D 点, 视差角大时用双目恢复 3D 点 （双目以及深度有效）
            ...
            // 当双目在先前没有检测出匹配的点时候, 误差为二维, 当有右目匹配点时候误差为三维
            // 依据卡方检测经典阈值 0.05 对应二维系数和三维系数分别为: 5.991 / 7.8
            if(!bStereo1)
            {
            ...
            // 世界坐标系下, 3D 点与相机间的向量, 方向由相机指向 3D 点
...
void LocalMapping::SearchInNeighbors()
{
    ...
    // 步骤2: 将当前的 MapPoint 分别与一级二级相邻帧 (的MapPoint) 进行融合
...
void LocalMapping::KeyFrameCulling()
{
    ...
    // 步骤1: 根据 Covisibility Graph 提取当前帧的共视关键帧
    ...
    for(vector<KeyFrame*>::iterator vit=vpLocalKeyFrames.begin(), vend=vpLocalKeyFrames.end(); vit!=vend; vit++)
    {
        ...
        // 步骤3: 遍历局部关键帧的 MapPoints, 判断是否 90% 以上的 MapPoint 能被其他关键帧 (至少3个) 观测到
