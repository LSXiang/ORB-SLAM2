...
void LoopClosing::Run()
{
    ...
    while(1)
    {
        ...
        // LoopClosing 中的关键帧是 LocalMapping 发送过来的, LocalMapping是 Tracking 中发送过来的
...
bool LoopClosing::DetectLoop()
{
    ...
    // ConsistentGroup.first 对应每个“连续组”中的关键帧, ConsistentGroup.second 为每个“连续组”的序号
    ...
    for(size_t i=0, iend=vpCandidateKFs.size(); i<iend; i++)
    {
        ...
        for(size_t iG=0, iendG=mvConsistentGroups.size(); iG<iendG; iG++)
        {
            ...
            // 遍历每一个"子候选组", 检测候选组中每一个关键帧在"子连续组"中是否存在
            // 如果有一帧共同存在于"子候选组"与之前的"子连续帧", 那么"子候选组"与该"子连续组"连续
        ...
        // 于是就把"子候选组"全部拷贝到 vCurrentConsistentGroups, 并最终用于更新 mvConsistentGroups, 计数器设为0, 重新开始
...
bool LoopClosing::ComputeSim3()
{
    ...
    for(int i=0; i<nInitialCandidates; i++)
    {
        ...
        // 步骤2: 将当前帧 mpCurrentKF 与闭环候选帧 pKF 匹配
        // 通过 BoW 加速得到 mpCurrentKF 与 pKF 之间的匹配特征点, vvpMapPointMatches 是匹配特征点对应的 MapPoints
        ...
        else
        {
            ...
            pSolver->SetRansacParameters(0.99,20,300);  // 99%的概率选到内点 至少20个inliers 300次迭代
...
void LoopClosing::CorrectLoop()
{
    ...
    // 回环结束后将回环暂停局部地图时缓存的的待处理关键帧去掉, 并且重启局部地图线程
...
void LoopClosing::RunGlobalBundleAdjustment(unsigned long nLoopKF)
{
    ...
    // 更新所以的 MapPoints 和 KeyFrames 的位姿
    // Local Mapping 在 BA 期间处于活动状态, 这意味着可能存在未包含在全局 BA 中的新关键帧,并且它们与更新的映射不一致。
    // 我们需要通过生长树传播修正
    {
        unique_lock<mutex> lock(mMutexGBA);
        if(idx!=mnFullBAIdx)
        ...
        if(!mbStopGBA)
        {
            ...
            // 从添加到地图的第一帧关键帧开始校正每一关键帧的位姿
            list<KeyFrame*> lpKFtoCheck(mpMap->mvpKeyFrameOrigins.begin(),mpMap->mvpKeyFrameOrigins.end());

            while(!lpKFtoCheck.empty())
            {
                KeyFrame* pKF = lpKFtoCheck.front();  // 获取最初的关键帧
                const set<KeyFrame*> sChilds = pKF->GetChilds();  // 获取生长树中该帧的所有子关键帧
                cv::Mat Twc = pKF->GetPoseInverse();
                for(set<KeyFrame*>::const_iterator sit=sChilds.begin();sit!=sChilds.end();sit++)
                {
                    KeyFrame* pChild = *sit;
                    // 如果存在子关键帧不在全局 BA 中进行位姿修正的, 那么就行位姿传播修正, 否则添加近检序列
            ...
            for(size_t i=0; i<vpMPs.size(); i++)
            {
                ...
                if(pMP->mnBAGlobalForKF==nLoopKF) // 该空间点坐标已经在全局 BA 中优化过
                {
                    // If optimized by Global BA, just update
                    pMP->SetWorldPos(pMP->mPosGBA);
                }
                else  // 未在全局 BA 中优化过, 则通过它自己的参考关键帧进行位置更新
                {
                    // Update according to the correction of its reference keyframe
                    KeyFrame* pRefKF = pMP->GetReferenceKeyFrame();

                    if(pRefKF->mnBAGlobalForKF!=nLoopKF)  // 如果该空间点的参考关键帧未进行 BA 优化, 则不更新该位姿
                        continue;
